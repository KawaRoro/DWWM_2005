...
class Employee
{
    constructor(_id, _lastname, _firstname, _role, _salary, _hiredate) {
        ...
        this.id = parseInt(_id); // création d'un attribut "id"

        this.lastname = String(_lastname) || "Anonymous"; // création d'un attribut "lastname"

        this.firstname = String(_firstname) || "Anonymous"; // création d'un attribut "firstname"

        this.createEmail = function(_lastname, _firstname) {
            let short_firstname = _firstname.substr(0, 1).toLocaleLowerCase(); // première lettre uniquement
            let str_email = short_firstname+_lastname.toLocaleLowerCase()+"@email.fr"; // composition de l'email
            return str_email;
        }

        this.email = this.createEmail(this.lastname, this.firstname); // création d'un attribut "email"

        this.role = _role || "Anonymous"; // création d'un attribut "role"
        
        this.salary = parseInt(_salary); // création d'un attribut "salary"
        this.getMonthlySalary = function(_salary) {
            let monthSalary = _salary / 12 * 0.75;
            return monthSalary;
        }

        this.createObjDate = function(_hiredate) {
            let order_year_hiredate = _hiredate.getFullYear(); //
            let order_month_hiredate = _hiredate.getMonth()+1; // 
            let order_day_hiredate = _hiredate.getDate(); // 
            
            _hiredate = order_year_hiredate +"-"+ order_month_hiredate +"-"+ order_day_hiredate;

            return _hiredate;
        }

        this.hiredate = this.createObjDate(_hiredate); // création d'un attribut "hiredate"

        this.getDaySeniority = function(_hiredate) {
            //console.log(hiredate);
            let between_date_days = Date.now() - new Date(_hiredate).getTime();
            between_date_days = between_date_days / (1000 * 3600 * 24) 
            return between_date_days;
        }

        this.getSeniority = function(_hiredate) {
            ...
            let between_date = Date.now() - new Date(_hiredate).getTime(); // 
            ...
            if(result_date_in_days > 365.25){
                result_date_in_year = result_date_in_days / 365.25;
...
var employee1 = new Employee(1, 'Doe', 'John', 'manager', 82000, new Date('1982-12-28')); // création d'un nouvel employé
var employee2 = new Employee(2, 'Jenniard', 'Johnathan', 'première classe', 65000, new Date('1520-05-27'));
var employee3 = new Employee(3, 'Schoenmaeker', 'Benjamin', 'administrateur', 200000, new Date('2019-03-18'));
var employee4 = new Employee(4, 'Hamza', 'Réda', 'graphiste', 82000, new Date('2019-11-26'));
var employee5 = new Employee(5, 'Boudier', 'Aurélien', 'salarié', 70000, new Date('2020-05-20'));
var employee6 = new Employee(6, 'Boudier', 'Aurélien', 'salarié', 70000, new Date('2020-05-27'));
